...
struct frontswap_ops {
	...
	int (*load_async)(unsigned, pgoff_t, struct page *); /* asyncly load a page */
	int (*load_early_map)(pgoff_t, struct page *, u64, struct vm_area_struct*, pte_t*, pte_t); /* async load a page */
...
static inline int frontswap_load_early_map(struct page *page, u64 vaddr,struct vm_area_struct *vma, pte_t* ptep, pte_t orig_pte)
{
	if (frontswap_enabled())
		return __frontswap_load_early_map(page,vaddr,vma,ptep,orig_pte);
