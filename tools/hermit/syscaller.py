...
def set_sthd_cores():
    ...
    syscall = libc.syscall
    syscall.restype = ctypes.c_int
    syscall.argtypes = ctypes.c_long, ctypes.c_int, ctypes.POINTER(ctypes.c_int), ctypes.c_long

    op_type = 1 # HERMIT_SET_STHD_CORES

    num_elems = 4
    sthd_cores = (ctypes.c_int * num_elems)()
    sthd_cores[0] = 53
    sthd_cores[1] = 55

    syscall(SYS_HERMIT_DBG, op_type, ctypes.cast(sthd_cores, ctypes.POINTER(ctypes.c_int)), num_elems * 4)
