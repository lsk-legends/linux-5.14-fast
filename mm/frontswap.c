...
int __frontswap_load_early_map(struct page *page, u64 vaddr,struct vm_area_struct *vma, pte_t* ptep, pte_t orig_pte)
{
	int ret = -1;
	swp_entry_t entry = { .val = page_private(page), };
	int type = swp_type(entry);
	...
	pgoff_t offset = swp_offset(entry);
	struct frontswap_ops *ops;
	//printk("[debug] : page 0x%lx, vma 0x%lx, ptep 0x%lx, type %d ,sis 0x%lx",page,vma,ptep,type,sis);
	VM_BUG_ON(!frontswap_ops);
	VM_BUG_ON(!PageLocked(page));
	VM_BUG_ON(sis == NULL);
	...
	/* Try loading from each implementation, until one succeeds. */
	for_each_frontswap_ops(ops) {
		ret = ops->load_early_map(offset, page, vaddr, vma, ptep, orig_pte);
		if (!ret) /* successful load */
			break;
	}
	// shengkai:
	...
	if (ret == 0) {
		//printk("[debug] early map req send success! page = %x",page);
		inc_frontswap_loads();
		if (frontswap_tmem_exclusive_gets_enabled) {
			SetPageDirty(page);
			__frontswap_clear(sis, offset);
...
EXPORT_SYMBOL(__frontswap_load_early_map);
