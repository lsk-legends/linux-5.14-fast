				  ...
				  gfp_t gfp_mask)
{
	...
	accum_adc_time_stat(ADC_FS_RECLAIM_DUR, pf_ts);
	accum_adc_time_stat(
		ADC_FS_RECLAIM_CNT,
		nr_reclaimed *
...
// YIFAN: ported from Fastswap's patch.
static void high_work_func(struct work_struct *work)
{
	int id = *(int *)(work + 1);
	struct mem_cgroup *memcg =
		container_of(work, struct mem_cgroup, high_works[id].high_work);
	unsigned long high = READ_ONCE(memcg->memory.high);
	unsigned long nr_pages = page_counter_read(&memcg->memory);

	if (fs_enabled) // YIFAN: jump to fastswap's logic when fs_enabled=true
...
fastswap_offload:
	// fastswap's offloading logic. only enabled when fs_enabled is set
	if (nr_pages > high) {
		int nr_reclaimed;
...
retry:
	...
	adc_pf_breakdown_stt(pf_breakdown, ADC_PAGE_RECLAIM,
			     get_cycles_start());
	psi_memstall_enter(&pflags);
	nr_reclaimed = try_to_free_mem_cgroup_pages_profiling(
		mem_over_limit, nr_pages, gfp_mask, may_swap, adc_pf_bits,
		pf_breakdown);
	psi_memstall_leave(&pflags);
...
done_restock:
	...
	do {
		...
		if (in_interrupt()) {
			if (mem_high) {
				// YIFAN: high_works[0] is the default one
				schedule_work(&memcg->high_works[0].high_work);
				break;
			}
			continue;
		}
...
fastswap_offload:
	do { // YIFAN: ported from Fastswap's patch
		unsigned long high_limit = READ_ONCE(memcg->memory.high);
		unsigned long curr_pages = page_counter_read(&memcg->memory);
		...
		// fastswap always offloads page reclamation if mem_high
		if (mem_high || in_interrupt()) {
			// YIFAN: offload if memory usage exceed headroom
			schedule_work_on(
				get_fs_core(),
		...
		if (mem_high || swap_high) {
			 ...
			 * based on how much each task is actually allocating.
			 */
			// YIFAN: note that here we delay the reclamation work
			// to ret_to_user stage which is at the end of the page
			// fault handling. We must count latency for that part
			// as well. And essentially such delay won't reduce
			// page fault latency!
			current->memcg_nr_pages_over_high += batch;
			set_notify_resume(current);
			break;
				 ...
				 unsigned long max, bool memsw)
{
	...
	// YIFAN: a hack for fastswap to set memory->high
	unsigned long fs_high = max - fs_headroom;
	// YIFAN: dirty hack to let cgroup ignore the hardlimit
	if (!fs_hardlimit)
...
static struct mem_cgroup *mem_cgroup_alloc(void)
{
	...
	for (i = 0; i < 32; i++) {
		INIT_WORK(&memcg->high_works[i].high_work, high_work_func);
		memcg->high_works[i].work_id = i;
	}
...
static void mem_cgroup_css_free(struct cgroup_subsys_state *css)
{
	...
	for (i = 0; i < 32; i++) {
		cancel_work_sync(&memcg->high_works[i].high_work);
	}
					    ...
					    uint64_t pf_breakdown[])
{
	...
	adc_pf_breakdown_end(pf_breakdown, ADC_CGROUP_ACCOUNT,
			     get_cycles_end());
	accum_adc_time_stat(ADC_SWAP_OUT_DUR, get_cycles_end() - pf_ts);
	return ret;
}
...
{
	return mem_cgroup_swapin_charge_page_profiling(page, mm, gfp, entry, NULL, NULL);
}
