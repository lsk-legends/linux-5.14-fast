				  ...
				  uint64_t pf_breakdown[])
{
	...
	if (unlikely(!PageUptodate(page))) {
		printk("[debug] : early map pte without page uptodate!");
		ret = VM_FAULT_SIGBUS;
		goto out_nomap;
	}
	...
	adc_pf_breakdown_stt(pf_breakdown, ADC_SETPTE, get_cycles_start());
	set_pte_at(vma->vm_mm, vmf->address, vmf->pte, pte);
...
out:
	if (si)
		put_swap_device(si);
	//TODO profiling here
	//printk("[debug] : map pte of page 0x%lx finish!, %d", page, ret & VM_FAULT_ERROR);
	return ret;
out_nomap:
	pte_unmap_unlock(vmf->pte, vmf->ptl);
out_page:
				  ...
				  uint64_t pf_breakdown[])
{
	...
	if(cpu != -1){
		...
		//locked = trylock_page(page);
		//if (!locked) {
		//	printk("[debug] : can't lock after poll, fault in retry");
		//	ret |= VM_FAULT_RETRY;
		//	goto out_release;
		//}
	}
	...
	pf_ts = get_cycles_end();
	adc_pf_breakdown_end(pf_breakdown, ADC_UPD_METADATA, get_cycles_end());
	adc_pf_breakdown_stt(pf_breakdown, ADC_SETPTE, pf_ts);
	set_pte_at(vma->vm_mm, vmf->address, vmf->pte, pte);
	arch_do_swap_page(vma->vm_mm, vma, vmf->address, pte, vmf->orig_pte);
	vmf->orig_pte = pte;
