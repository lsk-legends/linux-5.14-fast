...
{
	// [Hermit] enable vma readahead regardless rotate_swap
				  ...
				  int *adc_pf_bits, uint64_t pf_breakdown[])
{
	...
	*new_page_allocated = false;
	...
	//printk("[debug] : async add to lru page 0x%lx", page);
	lru_cache_add(page);
		...
		struct vm_area_struct *vma, unsigned long addr, bool do_poll)
{
	...
	if (page_was_allocated) {
		int cpu = get_cpu();
		//printk("[debug] : read swapcache async cpu %d",cpu);
					      ...
					      uint64_t pf_breakdown[])
{
	...
	fault_page =
		__read_swap_cache_async_profiling(entry, gfp_mask, vma, addr,
						 ...
						 uint64_t pf_breakdown[])
{
	...
	// [RMGrid] issue demand page read first
	adc_pf_breakdown_stt(pf_breakdown, ADC_DEDUP_SWAPIN, get_cycles_start());
	fault_page =
		__read_swap_cache_async_profiling(fentry, gfp_mask, vma, vmf->address,
							...
							uint64_t pf_breakdown[])
{
	...
	*pagep = alloc_page_vma(GFP_HIGHUSER_MOVABLE, vma, vmf->address);
	...
	if (mem_cgroup_swapin_charge_page_profiling(
		...
		//printk("[debug] : swapin bypass swapcache charge fail");
		goto oom;
	}

	//shengkai : question point why not uncharge, to prevent others 
	mem_cgroup_swapin_uncharge_swap(entry);
	...
	*cpu = get_cpu();
	...
	put_cpu();
	//printk("[debug] : swapin faddr 0x%lx bypass swapcache page 0x%lx", faddr,page);
	// [RMGrid] profiling
	set_adc_pf_bits(adc_pf_bits, ADC_PF_MAJOR_BIT);
	adc_profile_counter_inc(ADC_ONDEMAND_SWAPIN);
	...
	//debug point
	//goto done;
	//printk("[debug] : swapin ra readahead %d pages",ra_info->nr_pte);
	if(ra_info->win == 1)
		goto done;
	fcpu = *cpu;
	int i;
	for (i = 0, pte = ra_info->ptes; i < ra_info->nr_pte ;
		...
		//lock page and lru cache add here
		page = __read_swap_cache_async_profiling(
			entry, GFP_HIGHUSER_MOVABLE, vma, faddr,
			&page_allocated, adc_pf_bits, pf_breakdown);
		if (!page){
			//printk("[debug] : swap in can't alloc %d pages",i);
			break;
		}
    			//continue;
	...
	//printk("[debug] : prefetch %d pages!",i);
