				   ...
				   uint64_t pf_breakdown[])
{
	 ...
	 */
	adc_pf_breakdown_stt(pf_breakdown, ADC_WRITE_PAGE, get_cycles_start());
	...
	if (!PageAnon(page))
		cpu = HMT_INV_CORE;
	if (mapping->a_ops->writepage == NULL || mapping->a_ops->writepage_on_core == NULL) {
		ret = PAGE_ACTIVATE;
		//printk("[debug] : op write register error!");
		goto profiling;
	}
	...
	if (clear_page_dirty_for_io(page)) {
		...
		if (cpu == HMT_INV_CORE) // [Hermit] default sync write
			...
			res = mapping->a_ops->writepage_on_core(page, &wbc,cpu);
	...
	ret = PAGE_CLEAN;
profiling:
	adc_pf_breakdown_end(pf_breakdown, ADC_WRITE_PAGE, get_cycles_end());
	return ret;
}
			    ...
			    bool reclaimed, struct mem_cgroup *target_memcg)
{
	...
	// YIFAN: we simply comment out BUG_ON()s here for Fastswap.
	// BUG_ON(!PageLocked(page));
	// BUG_ON(mapping != page_mapping(page));
	if (!PageLocked(page)) {
		pr_err("%s:%d: YIFAN: page not locked!\n", __func__, __LINE__);
		goto cannot_free;
	}
	if (mapping != page_mapping(page)) {
		pr_err("%s:%d: YIFAN: mapping inconsistent!\n", __func__, __LINE__);
		goto cannot_free;
	}
	...
	while (!list_empty(page_list)) {
		...
		if (!ignore_references) {
			...
			adc_pf_breakdown_end(pf_breakdown, ADC_PG_CHECK_REF,
					     pf_ts);
			accum_adc_time_stat(ADC_RMAP_LAT, pf_ts);
		}
		...
		if (PageDirty(page)) {
			...
			if (PageAnon(page) && PageSwapBacked(page) && PageSwapCache(page)) {
				...
				//printk("[debug] : page 0x%lx add to dirty list",page);
				continue;
			}
...
keep:
	...
	nr_reclaimed += batched_pageout(&dirty_pages, &clean_pages, &free_pages,
					&ret_pages, pgdat, sc, stat,
					pf_breakdown);
	//shenhgkai : 
	//printk("[debug] : batchedout %d pages",nr_reclaimed);
			       ...
			       int *adc_pf_bits, uint64_t pf_breakdown[])
{
	...
	nr_reclaimed = shrink_page_list_profiling(
		&page_list, pgdat, sc, &stat, false, adc_pf_bits, pf_breakdown);
 ...
 */
static void shrink_active_list_profiling(unsigned long nr_to_scan,
					   struct lruvec *lruvec,
					   struct scan_control *sc,
					   enum lru_list lru,
					   uint64_t pf_breakdown[])
...
static inline unsigned long shrink_list(enum lru_list lru,
						  unsigned long nr_to_scan,
						  struct lruvec *lruvec,
						  struct scan_control *sc)
{
	return shrink_list_profiling(lru, nr_to_scan, lruvec, sc, NULL, NULL);
}
...
static void shrink_lruvec_profiling(struct lruvec *lruvec,
				    struct scan_control *sc, int *adc_pf_bits,
				    uint64_t pf_breakdown[])
{
	...
	if (total_swap_pages && inactive_is_low(lruvec, LRU_INACTIVE_ANON))
		shrink_active_list_profiling(SWAP_CLUSTER_MAX, lruvec, sc,
					     LRU_ACTIVE_ANON, pf_breakdown);
}

static inline void shrink_lruvec(struct lruvec *lruvec, struct scan_control *sc)
{
	return shrink_lruvec_profiling(lruvec, sc, NULL, NULL);
}
					 ...
					 uint64_t pf_breakdown[])
{
	...
	do {
		...
		shrink_lruvec_profiling(lruvec, sc, adc_pf_bits, pf_breakdown);
...
{
	return shrink_node_memcgs_profiling(pgdat, sc, NULL, NULL);
}
...
again:
	...
	shrink_node_memcgs_profiling(pgdat, sc, adc_pf_bits, pf_breakdown);
...
{
	return shrink_node_profiling(pgdat, sc, NULL, NULL);
}
				   ...
				   uint64_t pf_breakdown[])
{
	...
	for_each_zone_zonelist_nodemask(zone, z, zonelist,
		...
		shrink_node_profiling(zone->zone_pgdat, sc, adc_pf_bits,
				      pf_breakdown);
	}
...
{
	return shrink_zones_profiling(zonelist, sc, NULL, NULL);
}
...
retry:
	...
	do {
		vmpressure_prio(sc->gfp_mask, sc->target_mem_cgroup,
				sc->priority);
		sc->nr_scanned = 0;
		shrink_zones_profiling(zonelist, sc, adc_pf_bits, pf_breakdown);
...
static inline unsigned long do_try_to_free_pages(struct zonelist *zonelist,
					  struct scan_control *sc)
{
	return do_try_to_free_pages_profiling(zonelist, sc, NULL, NULL);
}
...
unsigned long try_to_free_mem_cgroup_pages_profiling(
	...
	bool may_swap, int *adc_pf_bits, uint64_t pf_breakdown[])
{
	...
	nr_reclaimed = do_try_to_free_pages_profiling(
		zonelist, &sc, adc_pf_bits, pf_breakdown);

	adc_profile_counter_add(nr_reclaimed, ADC_TOTAL_RECLAIM);
...
inline unsigned long try_to_free_mem_cgroup_pages(struct mem_cgroup *memcg,
					   unsigned long nr_pages,
					   gfp_t gfp_mask,
					   bool may_swap)
{
	return try_to_free_mem_cgroup_pages_profiling(memcg, nr_pages, gfp_mask,
						      may_swap, NULL, NULL);
}
#endif
